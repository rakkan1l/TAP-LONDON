    ...
    <main style={{ background: '#f9f7f2', minHeight: '100vh', fontFamily: "'DM Sans', sans-serif" }}>
      ...
      <section style={{ background: 'linear-gradient(135deg, #1a1a2e 0%, #0f2027 100%)', padding: '56px 20px 48px', textAlign: 'center' }}>
        <div style={{ maxWidth: '700px', margin: '0 auto' }}>
          <div style={{ display: 'inline-block', background: 'rgba(201,168,76,0.15)', border: '1px solid rgba(201,168,76,0.4)', color: '#c9a84c', borderRadius: '40px', padding: '6px 18px', fontSize: '0.72rem', fontWeight: 600, letterSpacing: '1.5px', marginBottom: '20px', textTransform: 'uppercase' }}>
      ...
      <div style={{ maxWidth: '1000px', margin: '0 auto', padding: '48px 20px' }}>
        ...
        {/* Qibla */}
        <div style={{ background: 'linear-gradient(135deg, rgba(201,168,76,0.12), rgba(26,26,46,0.06))', border: '1px solid rgba(201,168,76,0.3)', borderRadius: '18px', padding: '28px 24px', marginBottom: '56px', display: 'flex', alignItems: 'flex-start', gap: '20px', flexWrap: 'wrap' }}>
          <div style={{ fontSize: '3rem', flexShrink: 0 }}>🧭</div>
          <div>
            <h2 style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: '1.6rem', fontWeight: 700, color: '#1a1a2e', marginBottom: '8px' }}>Qibla Direction in London</h2>
            <p style={{ fontSize: '0.88rem', color: '#444', lineHeight: 1.7, marginBottom: '12px' }}>
        ...
        {/* Mosques */}
        <p style={{ fontSize: '0.7rem', color: '#c9a84c', fontWeight: 700, letterSpacing: '2px', textTransform: 'uppercase', marginBottom: '6px' }}>Places of Worship</p>
        <h2 style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: 'clamp(1.6rem, 4vw, 2.2rem)', color: '#1a1a2e', fontWeight: 700, marginBottom: '24px' }}>Mosques in London</h2>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: '18px', marginBottom: '56px' }}>
          {[
            ...
            <div key={m.name} style={{ background: '#ffffff', border: '1px solid rgba(26,26,46,0.1)', borderRadius: '16px', overflow: 'hidden', boxShadow: '0 2px 12px rgba(0,0,0,0.06)' }}>
              ...
              <div style={{ padding: '18px' }}>
                ...
                <h3 style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: '1.15rem', fontWeight: 700, color: '#1a1a2e', marginBottom: '6px' }}>{m.name}</h3>
                <div style={{ fontSize: '0.7rem', color: '#888', marginBottom: '8px' }}>🚇 {m.tube}</div>
                <p style={{ fontSize: '0.8rem', color: '#555', lineHeight: 1.55, marginBottom: '14px' }}>{m.desc}</p>
                <a href={m.maps} target="_blank" rel="noopener noreferrer" style={{ display: 'block', background: '#1a1a2e', color: '#c9a84c', padding: '9px 0', borderRadius: '8px', fontWeight: 600, fontSize: '0.78rem', textDecoration: 'none', textAlign: 'center' }}>
                  Get Directions →
                </a>
              </div>
            </div>
        ...
        {/* Prayer Rooms */}
        <p style={{ fontSize: '0.7rem', color: '#c9a84c', fontWeight: 700, letterSpacing: '2px', textTransform: 'uppercase', marginBottom: '6px' }}>Prayer Rooms</p>
        <h2 style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: 'clamp(1.6rem, 4vw, 2.2rem)', color: '#1a1a2e', fontWeight: 700, marginBottom: '24px' }}>Where to Pray in London</h2>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(240px, 1fr))', gap: '12px', marginBottom: '56px' }}>
          {[
            ...
            <div key={room.place} style={{ background: '#ffffff', border: '1px solid rgba(201,168,76,0.15)', borderRadius: '12px', padding: '16px 18px', display: 'flex', gap: '12px', alignItems: 'flex-start' }}>
              <span style={{ fontSize: '1.4rem', flexShrink: 0 }}>{room.icon}</span>
              <div>
                <div style={{ fontWeight: 700, fontSize: '0.87rem', color: '#1a1a2e', marginBottom: '4px' }}>{room.place}</div>
                <div style={{ fontSize: '0.76rem', color: '#666', lineHeight: 1.4 }}>{room.detail}</div>
              </div>
            </div>
          ))}
        </div>
        ...
        {/* Halal Food */}
        <p style={{ fontSize: '0.7rem', color: '#c9a84c', fontWeight: 700, letterSpacing: '2px', textTransform: 'uppercase', marginBottom: '6px' }}>Halal Dining</p>
        <h2 style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: 'clamp(1.6rem, 4vw, 2.2rem)', color: '#1a1a2e', fontWeight: 700, marginBottom: '12px' }}>Best Halal Restaurants</h2>
        <p style={{ fontSize: '0.8rem', color: '#888', marginBottom: '20px', background: 'rgba(201,168,76,0.08)', border: '1px solid rgba(201,168,76,0.2)', borderRadius: '10px', padding: '12px 16px' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: '16px', marginBottom: '56px' }}>
          {[
            ...
            { name: 'Dishoom', area: 'Multiple locations', cuisine: 'Bombay café', price: '££', halal: false, note: 'Confirm halal status with staff', desc: 'London\'s most loved Indian restaurant group. Famous for black daal, breakfast naan rolls and grills.', maps: 'https://www.google.com/maps/search/?api=1&query=Dishoom+London' },
          ].map((r) => (
            <div key={r.name} style={{ background: '#ffffff', border: `1px solid ${r.halal ? 'rgba(22,163,74,0.2)' : 'rgba(26,26,46,0.1)'}`, borderRadius: '14px', padding: '20px', boxShadow: '0 2px 8px rgba(0,0,0,0.04)' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '8px' }}>
                <div>
                  <div style={{ fontSize: '0.68rem', color: '#888', marginBottom: '3px' }}>📍 {r.area} · {r.cuisine} · {r.price}</div>
                  <h3 style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: '1.15rem', fontWeight: 700, color: '#1a1a2e' }}>{r.name}</h3>
                </div>
                <span style={{ background: r.halal ? 'rgba(22,163,74,0.1)' : 'rgba(245,158,11,0.1)', color: r.halal ? '#16a34a' : '#92400e', border: `1px solid ${r.halal ? 'rgba(22,163,74,0.3)' : 'rgba(245,158,11,0.3)'}`, borderRadius: '6px', padding: '3px 8px', fontSize: '0.65rem', fontWeight: 700, whiteSpace: 'nowrap', flexShrink: 0, marginLeft: '8px' }}>
                  {r.halal ? '✓ Halal' : '⚠️ Confirm'}
                </span>
              </div>
              <p style={{ fontSize: '0.8rem', color: '#555', lineHeight: 1.55, marginBottom: '14px' }}>{r.desc}</p>
              <a href={r.maps} target="_blank" rel="noopener noreferrer" style={{ display: 'block', background: '#1a1a2e', color: '#c9a84c', padding: '8px 0', borderRadius: '8px', fontWeight: 600, fontSize: '0.76rem', textDecoration: 'none', textAlign: 'center' }}>
                Find on Google Maps →
              </a>
            </div>
          ))}
        ...
        {/* Ramadan */}
        <p style={{ fontSize: '0.7rem', color: '#c9a84c', fontWeight: 700, letterSpacing: '2px', textTransform: 'uppercase', marginBottom: '6px' }}>Ramadan & Eid</p>
        <h2 style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: 'clamp(1.6rem, 4vw, 2.2rem)', color: '#1a1a2e', fontWeight: 700, marginBottom: '24px' }}>Ramadan in London</h2>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(260px, 1fr))', gap: '14px', marginBottom: '56px' }}>
          {[
            ...
            <div key={r.title} style={{ background: 'rgba(26,26,46,0.04)', border: '1px solid rgba(26,26,46,0.1)', borderRadius: '14px', padding: '20px' }}>
              <div style={{ fontSize: '1.6rem', marginBottom: '8px' }}>{r.icon}</div>
              <h3 style={{ fontWeight: 700, fontSize: '0.9rem', color: '#1a1a2e', marginBottom: '6px' }}>{r.title}</h3>
              <p style={{ fontSize: '0.8rem', color: '#555', lineHeight: 1.55, margin: 0 }}>{r.desc}</p>
            </div>
          ))}
        </div>

        {/* Halal Shopping */}
        <p style={{ fontSize: '0.7rem', color: '#c9a84c', fontWeight: 700, letterSpacing: '2px', textTransform: 'uppercase', marginBottom: '6px' }}>Muslim-Friendly Shopping</p>
        <h2 style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: 'clamp(1.6rem, 4vw, 2.2rem)', color: '#1a1a2e', fontWeight: 700, marginBottom: '24px' }}>Halal Shopping Areas</h2>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(260px, 1fr))', gap: '14px', marginBottom: '48px' }}>
          {[
            ...
            <div key={s.name} style={{ background: '#ffffff', border: '1px solid rgba(201,168,76,0.15)', borderRadius: '12px', padding: '18px' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '6px' }}>
                <h3 style={{ fontWeight: 700, fontSize: '0.9rem', color: '#1a1a2e' }}>{s.name}</h3>
                <span style={{ fontSize: '0.68rem', color: '#c9a84c', fontWeight: 600, flexShrink: 0, marginLeft: '8px' }}>{s.area}</span>
              </div>
              <p style={{ fontSize: '0.78rem', color: '#666', lineHeight: 1.5, margin: 0 }}>{s.desc}</p>
            </div>
          ))}
        </div>
